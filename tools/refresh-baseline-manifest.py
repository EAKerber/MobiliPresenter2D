...
def image_record(path: Path) -> dict[str, object]:
    ...
    return {**file_record(path), "dimensions": dimensions, "alphaBounds": list(bbox) if bbox else None, "canonicalCanvas": dimensions == [1536, 1024]}

def main() -> int:
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    manifest["baselineId"] = "cozinha-01-r5a-pixelperfect-bridges1"
    manifest["files"] = [file_record(path) for path in sorted(APP.rglob("*")) if path.is_file() and "dist" not in path.parts]
    asset_paths = sorted((APP / "assets" / "kitchen").rglob("*.png")) + sorted((APP / "tools" / "sources").glob("*.png"))
    ...
    print(json.dumps({"status": "PASS", "baselineId": manifest["baselineId"], "files": len(manifest["files"]), "assets": len(manifest["assets"])}, sort_keys=True))
    return 0
