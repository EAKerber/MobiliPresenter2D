(function (global) {
  ...
  function createRenderer(stoneCanvas, plinthCanvas, data) {
    ...
    return async function render(state, materials = {}) {
      ...
      const requestedId = caseId(state);
      // A stale browser state must not turn a visual refresh into a fatal
      // error. The canonical default is safer than painting an unknown case.
      const id = data[requestedId] ? requestedId : "default";
      try {
        const jobs = [];
