...
(async () => {
  ...
  const canvasStats = () => page.evaluate(() => {
    const c=document.getElementById("reconstructionCanvas-bmc01");
    if(!c) return null;
  ...
  assert(await page.locator("#reconstructionCanvas-bmc01").count()===1,"research canvas not installed");
  ...
  await page.waitForFunction(previous => Number(document.getElementById("reconstructionCanvas-bmc01").dataset.renderRevision||0) > previous, base.revision);
  ...
  await page.waitForFunction(previous => Number(document.getElementById("reconstructionCanvas-bmc01").dataset.renderRevision||0) > previous, dark.revision);
  const stoneSelected=await canvasStats();
  await page.locator("#stoneSkirtingToggle").check();
  await page.waitForFunction(previous => Number(document.getElementById("reconstructionCanvas-bmc01").dataset.renderRevision||0) > previous, stoneSelected.revision);
  ...
  await page.waitForFunction(() => document.getElementById("reconstructionCanvas-bmc01").dataset.active==="false");
  ...
  assert.equal(await ordinary.locator("#reconstructionCanvas-bmc01").count(),0,"default app unexpectedly enabled research reconstruction");
